...
collect_r_defs <- function(r_files) {
  ...
  for (rf in r_files) {
    ...
    for (idx in seq_along(exprs)) {
      ...
      sr <- if (!is.null(srcrefs) && idx <= length(srcrefs)) srcrefs[[idx]] else NULL
      ...
      # Capture the function body as deparsed source
      body_lines <- tryCatch({
        deparsed <- deparse(rhs, control = c("keepInteger", "keepNA", "showAttributes"))
        paste(deparsed, collapse = "\n")
      }, error = function(e) NULL)

      # Extract internal function calls (pkg:::helper or bare helper calls)
      internal_calls <- character()
      if (!is.null(body_lines)) {
        # pkg:::private_fn calls
        triple_m <- gregexpr("([A-Za-z][A-Za-z0-9._]+):::([A-Za-z][A-Za-z0-9._]+)", body_lines, perl = TRUE)
        triple_hits <- regmatches(body_lines, triple_m)[[1]]
        # pkg::public_fn calls
        double_m <- gregexpr("([A-Za-z][A-Za-z0-9._]+)::([A-Za-z][A-Za-z0-9._]+)", body_lines, perl = TRUE)
        double_hits <- regmatches(body_lines, double_m)[[1]]
        internal_calls <- unique(c(triple_hits, double_hits))
      }
...
# sources.jsonl — function bodies for eco_source tool
source_records <- list()
for (nm in exports) {
  def <- defs[[nm]]
  if (!is.null(def$body)) {
    rec <- list(
      ...
      body = def$body,
      source = def$source %||% list(path = NULL, lines = NULL),
      internal_calls = I(if (length(def$internal_calls) > 0) def$internal_calls else character())
    )
    source_records[[length(source_records) + 1L]] <- rec
  }
}
write_jsonl(file.path(out_dir, "sources.jsonl"), source_records)
...
cat(sprintf("EcoAtlas extract complete: %s symbols, %s sources, %s snippets\n", length(symbols), length(source_records), length(snip_records)))
