...
parse_namespace_exports <- function(ns_path = "NAMESPACE") {
  ...
  matches <- gregexpr("export\\s*\\(([^)]*)\\)", raw, perl = TRUE)[[1]]
  if (length(matches) == 1L && matches[[1]] == -1L) return(character())
  chunks <- regmatches(raw, list(matches))[[1]]

  for (chunk in chunks) {
    inside <- sub("^\\s*export\\s*\\(", "", chunk)
    inside <- sub("\\)\\s*$", "", inside)
...
extract_roxygen_summary <- function(file_lines, func_start_line) {
  ...
  title_idx <- grep("^@title\\s+", roxy)
  if (length(title_idx) > 0) {
    title <- trimws(sub("^@title\\s+", "", roxy[[title_idx[[1]]]]))
    if (nzchar(title)) return(title)
  }

  desc_idx <- grep("^@description\\s*", roxy)
  if (length(desc_idx) > 0) {
    i <- desc_idx[[1]]
    first <- trimws(sub("^@description\\s*", "", roxy[[i]]))
    desc_lines <- character()
    if (nzchar(first)) desc_lines <- c(desc_lines, first)
    j <- i + 1L
    while (j <= length(roxy)) {
      ln <- roxy[[j]]
      if (grepl("^@", ln)) break
      if (nzchar(ln)) desc_lines <- c(desc_lines, ln)
      j <- j + 1L
    }
    if (length(desc_lines) > 0) return(paste(desc_lines, collapse = " "))
  }
...
find_assignment_start_line <- function(file_lines, nm) {
  if (length(file_lines) == 0 || !nzchar(nm)) return(NA_integer_)
  nm_re <- gsub("([.|()\\^{}+$*?\\[\\]\\\\])", "\\\\\\1", nm)
  pat <- paste0("^\\s*", nm_re, "\\s*(<-|=)\\s*function\\s*\\(")
  hits <- grep(pat, file_lines, perl = TRUE)
  if (length(hits) == 0) return(NA_integer_)
  as.integer(hits[[1]])
}

infer_function_end_line <- function(file_lines, start_line) {
  if (is.na(start_line) || start_line < 1 || start_line > length(file_lines)) return(NA_integer_)

  open_count <- 0L
  saw_open <- FALSE
  for (i in seq.int(start_line, length(file_lines))) {
    ln <- file_lines[[i]]
    open_n <- lengths(regmatches(ln, gregexpr("\\{", ln, perl = TRUE)))
    close_n <- lengths(regmatches(ln, gregexpr("\\}", ln, perl = TRUE)))
    if (open_n > 0L) saw_open <- TRUE
    open_count <- open_count + open_n - close_n
    if (saw_open && open_count <= 0L) return(as.integer(i))
  }

  as.integer(start_line)
}
...
collect_r_defs <- function(r_files) {
  ...
  for (rf in r_files) {
    ...
    for (ex in exprs) {
      ...
      if (is.na(start_line)) {
        start_line <- find_assignment_start_line(file_lines, nm)
      }
      if (is.na(end_line) && !is.na(start_line)) {
        end_line <- infer_function_end_line(file_lines, start_line)
      }
